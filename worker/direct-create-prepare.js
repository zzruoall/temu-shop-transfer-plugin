/** 列表页无界面生成新增请求；只读取目标店模板，不创建草稿、不提交商品。 */
(async function prepareDirectCreate(source) {
     ...
     * 平台上并不存在的失败（例如把"供电方式"的条件必填属性当成无条件必填，导致按摩油被要求填电池）。
     * 因此这些项只写进 notes 供排查，能否创建交给平台判断，平台报错才是真实原因。
     */
    const warnings = [];
    ...
    for (const warning of new Set(warnings)) notes.push(`待平台确认：${warning}`);
    // chrome.scripting.executeScript 只能回传可结构化克隆的数据；store/values 含实例方法和循环引用，
    // 只留在页面预检内部使用。把它们一起返回会让整次注入结果无法回传，表现为“页面未返回结果”。
    return { request, blockers: [], warnings: [...new Set(warnings)], notes, requiresComplianceConfirmation: Boolean(request.productComplianceStatementReq) };
})
